%{
This code implements the nonlinear Bayesian filter with uncertain loads.
...
Su = {sparse([1, 2], [1, 8], [1, 1], 2, 8);    % displacement selection
      sparse([1, 2], [1, 8], [1, 1], 2, 8);    % matrixes [-]
      sparse(0, 8);
      sparse(0, 8)};
Sv = {sparse(     1,      6,      1, 1, 8);    % velocity selection
      sparse(0, 8);                            % matrixes [-]
      sparse(     1,      6,      1, 1, 8);
      sparse(0, 8)};
Sa = {sparse(     1,      3,      1, 1, 8);    % acceleration selection
      sparse(0, 8);                            % matrixes [-]
      sparse(0, 8);
      sparse(     1,      3,      1, 1, 8)};
...
for i = 1:N_analy
    ...
    theta{5}  = alpha_BW;
    theta{19} = eta_u;
    [x, y{i}] = get_data(noise_ratio, theta);
    ...
    [mx{i}, Px{i}, mu{i}, Pu{i}] = this_filter(y{i}, mx_0, Px_0, mu_0, Pu_0, ...
                                               E, Q, R{i}, kappa, alpha,     ...
                                               beta, theta);
    % 4) evaluate the RMSE -- Eq.83
    xRRMSE(:, i) = rmse(mx{i},   x, 2)./max(  x, [], 2);    % [m], [m/s], [m]
    uRRMSE(:, i) = rmse(mu{i}, u_u, 2)./max(u_u, [], 2);    % [kN]

    % 5) plot the filter's estimation
    plot_estimation(  x, y{i}, t, mx{i}, Px{i}, {Su{i}; Sv{i}; sparse(0, N_DOFs)})
...
%% REPORT:
compare_estimation(  x, [], t, mx, {sparse(0, N_DOFs); sparse(0, N_DOFs); sparse(0, N_DOFs)}, ...
                   ["All", "u(t)", "v(t)", "a(t)"])
compare_estimation(u_u, [], t, mu, {sparse(0, nu)}, ...
                   ["All", "u(t)", "v(t)", "a(t)"])

print_RMSE(xRRMSE, ["All", "u(t)", "v(t)", "a(t)"])
print_RMSE(uRRMSE, ["All", "u(t)", "v(t)", "a(t)"])